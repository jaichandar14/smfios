...
public class SegmentedControl: UIControl {
    ...
    static let bottomLineThumbViewWidth: CGFloat = 75.0
    ...
    @IBInspectable public var selectedTitleFont: UIFont? {
        didSet {
            updateView()
        }
    }
    ...
    private func setThumbView() {
        ...
        let thumbViewHeight = bottomLineThumbView ? SegmentedControl.bottomLineThumbViewHeight : bounds.height - padding * 2
        let thumbViewWidth = fillEqually ? SegmentedControl.bottomLineThumbViewWidth : bounds.height - padding * 2 //(bounds.width / CGFloat(buttons.count)) - padding * 2 : bounds.height - padding * 2
        let thumbViewPositionX = (bounds.width / CGFloat(buttons.count))/2 - (thumbViewWidth/2) //padding
        //thumbView.center =
    ...
    private func setButtonsWithText() {
        ...
        buttons[selectedSegmentIndex].titleLabel?.font = selectedTitleFont
    }
    ...
    private func setButtonsWithImages() {
        ...
        for buttonImage in self.buttonImages {
            ...
            button?.imageEdgeInsets = SegmentedControl.imageInsets
            button?.addTarget(self, action: #selector(buttonTapped(button:)), for: .touchUpInside)
            buttons.append(button!)
        }
        if !buttonsWithDynamicImages {
